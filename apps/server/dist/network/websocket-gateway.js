...
export function createWebSocketGateway(gameStateStore, inputBufferStore, playerSessionStore, snapshotHistoryStore) {
    ...
    wss.on('connection', (socket) => {
        ...
        const welcome = { type: 'welcome', entityId: entity.id };
        ...
        socket.on('message', (rawData) => {
            ...
            switch (message.type) {
                ...
                case 'shoot': {
                    ...
                    if (!session)
                        return;
                    const rewindTicks = calculateRewindTicks(session.estimatedRttMs, SIMULATION_CONFIG.INTERPOLATION_DELAY_MS || 100, SIMULATION_CONFIG.FIXED_DELTA_TIME_MS);
                    const hitEntityId = performRewoundHitTest({
                        shooterEntityId: entity.id,
                        shooterTick: gameStateStore.getCurrentTick(),
                        origin: message.origin,
                        direction: message.direction,
                    }, rewindTicks, snapshotHistoryStore);
                    if (hitEntityId) {
                        const hitConfirmed = {
                            type: 'hit_confirmed',
